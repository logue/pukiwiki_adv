<?php
// GS2 skin color settings
// SKY

define('SKIN_CSS_CTS_BGCOLOR', 	'#CBD5E6');	// contents box bg
define('SKIN_CSS_CTS_BGCOLOR2',	'#CBD5E6');	// contents box bg2
define('SKIN_CSS_CTS_BDCOLOR', 	'#CBD5E6');	// contents box border

define('SKIN_CSS_BGCOLOR', 		'#D0DBEB');	// background
define('SKIN_CSS_FGCOLOR', 		'#060606');	// foreground

define('SKIN_CSS_BOX_BGCOLOR', 	'#CBD5E6');	// normal box bg
define('SKIN_CSS_BOX_BDCOLOR', 	'#C2CDDC');	// normal box border

define('SKIN_CSS_PRE_BGCOLOR', 	'#EEEEEE');	// pre bg
define('SKIN_CSS_PRE_BDCOLOR', 	'#888899');	// pre border

define('SKIN_CSS_H1_BGCOLOR', 	'#CBD5E6');	// heading1 bg
define('SKIN_CSS_H2_BGCOLOR', 	'#D0DBEB');	// heading2 bg
define('SKIN_CSS_H3_BGCOLOR', 	'#D0DBEB');	// heading3 bg
define('SKIN_CSS_H4_BGCOLOR', 	'#D0DBEB');	// heading4 bg
define('SKIN_CSS_H5_BGCOLOR', 	'#D0DBEB');	// heading5 bg
define('SKIN_CSS_H6_BGCOLOR', 	'#D0DBEB');	// heading6 bg
define('SKIN_CSS_H1_FGCOLOR', 	'#00000F');	// heading1 fg
define('SKIN_CSS_H2_FGCOLOR', 	'#00000F');	// heading2 fg
define('SKIN_CSS_H3_FGCOLOR', 	'#00000F');	// heading3 fg
define('SKIN_CSS_H4_FGCOLOR', 	'#00000F');	// heading4 fg
define('SKIN_CSS_H5_FGCOLOR', 	'#00000F');	// heading5 fg
define('SKIN_CSS_H6_FGCOLOR', 	'#00000F');	// heading6 fg
define('SKIN_CSS_H1_BDCOLOR', 	'#B1BAC8');	// heading1 border
define('SKIN_CSS_H2_BDCOLOR', 	'#B1BAC8');	// heading2 border
define('SKIN_CSS_H3_BDCOLOR', 	'#B1BAC8');	// heading3 border
define('SKIN_CSS_H4_BDCOLOR', 	'#B1BAC8');	// heading4 border
define('SKIN_CSS_H5_BDCOLOR', 	'#B1BAC8');	// heading5 border
define('SKIN_CSS_H6_BDCOLOR', 	'#B1BAC8');	// heading6 border

define('SKIN_CSS_A_LINK',	 	'#215dc6');	// a link
define('SKIN_CSS_A_VISITED', 	'#06358A');	// a visited
define('SKIN_CSS_A_HOVER',	 	'#268671');	// a hover
define('SKIN_CSS_A_ACTIVE',	 	'#c6215d');	// a active

return 'cupertino';			// jQuery UI Theme
