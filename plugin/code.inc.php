...
// 画像ファイルの設定
define('PLUGIN_CODE_IMAGE_FILE', IMAGE_URI.'plugin/code/code_dot.png');

define('PLUGIN_CODE_OUTLINE_OPEN_FILE',  IMAGE_URI.'plugin/code/code_outline_open.png');
define('PLUGIN_CODE_OUTLINE_CLOSE_FILE', IMAGE_URI.'plugin/code/code_outline_close.png');
...
function plugin_code_convert()
{
	...
	if ($plugin_code_jscript_flag && ($option['outline'] || $option['comment'])) {
		...
		$head_tags[] = '<script type="text/javascript" src="'.SKIN_URI.'js/plugin/code.js"></script>';
	}
	$html = $title.$lines;
	if (PLUGIN_CODE_CACHE && ! $multiline) {
