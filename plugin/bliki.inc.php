...
function plugin_bliki_init()
{
	...
	$msg = array(
		'_bliki_msg' => array(
			'msg_more'			=> T_('&size(12){->[[The continuation of %s is read.:%s]]};'),	// 「もっと読む」リンクの文言
			'msg_update'		=> T_('UPDATE'),
			'msg_cache_clear'	=> T_('bliki caches are cleared.'),
			'err_not_writable'	=> T_('%s is not found or not readable.')
		)
	);
...
{
	global $post, $vars, $_bliki_msg;
	$cache = isset($vars['cache']) ? $vars['cache'] : NULL;

	// this delete tracker caches. 
	if( $cache == 'DELALL' ) {
		if (! bliki_delete_caches('(.*)(.blc)$')) {
			die_message(sprintf($_bliki_msg['msg_not_writable'],CACHE_DIR));
		}

		return array(
			'result' => FALSE,
			'msg'	=> $_bliki_msg['msg_cache_clear'],
			'body'	=> $_bliki_msg['msg_cache_clear']
		);
	}
}
