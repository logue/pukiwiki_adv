...
JSMML.init = function(swfurl) {
	if (! document.getElementById(JSMML.mmlDivID)) {
		...
		if (jQuery){
			$('body').append('<div id="'+JSMML.mmlID+'"></div>');
		}else{
		...
		if (!document.location.protocol.match(/http/i)){
			document.getElementById(JSMML.mmlID).innerHTML = "JSMML is not running!<br />Please execute this in HTTP protocol.";
		}else{
			// init
...
JSMML.prototype = {
	...
	*/
	...
	setMasterVolume: function(){
		return this.mmlPlayer._setMasterVolume(this.uNum());
	},
