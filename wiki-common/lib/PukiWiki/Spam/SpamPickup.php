<?php
// $Id: SpamPickup.php,v 1.6 2011/01/24 15:19:36 henoheno Exp $
...
class SpamPickup{
	...
	static function uri_pickup($string = '')
	{
		...
		foreach(array_keys($array) as $uri) {
			$_uri = & $array[$uri];
			SpamUtility::array_rename_keys($_uri, $parts, TRUE, $default);
	...
	static function uri_pickup_normalize(& $pickups, $destructive = TRUE, $pathfile = FALSE)
	{
		...
		if ($destructive) {
			foreach (array_keys($pickups) as $key) {
				$_key = & $pickups[$key];
				$_key['scheme']   = isset($_key['scheme'])   ? self::scheme_normalize($_key['scheme']) : '';
				$_key['host']     = isset($_key['host'])     ? self::host_normalize($_key['host'])     : '';
				$_key['port']     = isset($_key['port'])     ? self::port_normalize($_key['port'], $_key['scheme'], FALSE) : '';
				$_key['path']     = isset($_key['path'])     ? strtolower(self::path_normalize($_key['path'])) : '';
				$_key['file']     = isset($_key['file'])     ? self::file_normalize($_key['file'])   : '';
				$_key['query']    = isset($_key['query'])    ? self::query_normalize($_key['query']) : '';
			...
			foreach (array_keys($pickups) as $key) {
				...
				$_key['port']     = isset($_key['port'])     ? self::port_normalize($_key['port'], $_key['scheme'], FALSE) : '';
				$_key['path']     = isset($_key['path'])     ? self::path_normalize($_key['path']) : '';
			}
		}

		if ($pathfile) {
			return self::uri_pickup_normalize_pathfile($pickups);
		} else {
			return $pickups;
		}
	}
	 ...
	 * @staticvar array $abbrevs Abbreviations they have no intention of link
	 * @staticvar array $aliases Aliases => normalized ones
	 * @param type $scheme
	...
	static function port_normalize($port, $scheme, $scheme_normalize = FALSE)
	{
		...
		$port = intval($port);
		if ($scheme_normalize) $scheme = self::scheme_normalize($scheme);
	 ...
	 * @staticvar array $simple_defaults
	 * @staticvar array $content_suffix
	 * @staticvar array $language_suffix
	 * @staticvar array $charset_suffix
	 * @staticvar array $encoding_suffix
	 * @param type $file
	 * @return string
	 */
	...
	static function spam_uri_pickup_preprocess($string = '', $method = array())
	{
		...
		$string = self::spam_uri_removing_hocus_pocus($string, $method);
	...
	static function whois_responsibility($fqdn = 'foo.bar.example.com', $parent = FALSE, $implicit = TRUE)
	{
		...
		if (SpamPickup::is_ip($fqdn)) return $fqdn;
...
/* End of file SpamPickup.php */
/* Location: ./vendor/PukiWiki/Spam/SpamPickup.php */
