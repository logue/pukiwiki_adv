...
/**************************************************************************************************/
	 ...
	 * @return boolean
	 */
	private function checkPermission($type = 'read', $username='', $groupname='')
	{
		...
		switch($auth_method_type) {
			case 'pagename':
				// ページ名で判断
				$target_str = $this->page;
				break;
			case 'contents':
				// ページのソースで判断
				$target_str = $this->page->get(true);
				break;
			default:
				throw new Exception('Wiki::checkPermission() : $auth_method_type is invalied!.');
				break;
		}
	 ...
	 * @return boolean
	 */
	public static function checkAcceptIp($accept_ips, $target_str){
	...
	public function isReadable($authenticate = false)
	{
		...
		// 許可IPの場合チェックしない
		if ( Auth::ip_auth($this->page, true, false, $read_auth_pages_accept_ip, $_title['cannotread'])) {
			return TRUE;
		}
		// 認証
