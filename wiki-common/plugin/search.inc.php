		...
		case 'xml' :
			...
			$ret = array(
				...
				'<OpenSearchDescription xmlns="http://a9.com/-/spec/opensearch/1.1/" xmlns:moz="http://www.mozilla.org/2006/browser/search/">',
					'<ShortName>' . $_search_msg['title_search'] .' - ' . $page_title . '</ShortName>',
					...
					'<Url type="text/html" method="' . ( (! PLUGIN_SEARCH_DISABLE_GET_ACCESS) ? 'get' : 'post' ) .'" template="' . get_script_uri() . '">',
					...
					'</Url>',
					'<Url type="application/x-suggestions+json" template="' . get_cmd_uri('list', null, null,  array('type'=>'json')) . '&amp;word={searchTerms}" />',
					'<moz:SearchForm>' . get_cmd_uri('search'). '</moz:SearchForm>',
